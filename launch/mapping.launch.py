...
def generate_launch_description():
    ...
    point_lio_slam_dir = get_package_share_directory('point-lio-slam')

    mapping_config_path = os.path.join(point_lio_slam_dir, 'config', 'mapping_indoor.yaml')
    with open(mapping_config_path, 'r') as file:
        mappingConfigParams = yaml.safe_load(file)['sam-qn_node']['ros__parameters']
    ...
    lioParams = [
        PathJoinSubstitution([
            FindPackageShare('point-lio-slam'),
            'config', 'lio_unilidar_l1.yaml'
        ]),
        {
            ...
            'filter_size_map': 0.4,  # Options: 0.5, 0.3, 0.15, 0.1
